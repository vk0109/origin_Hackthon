...
// Safely parse JSON responses; if server returned HTML/error page, return raw text under __raw
...
function Login({ onSignup, onClose }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const token = await firebaseUser.getIdToken(true); /* force refresh token to avoid stale/invalid token */

      // 4. Get role/phone/location saved during signup (temporary pending profile)
      const pending = JSON.parse(localStorage.getItem("resq_pending_profile") || "null");
      const savedRole = (pending && pending.role) || localStorage.getItem("resq_role") || "CITIZEN";
      const phoneValue = (pending && pending.phone) || localStorage.getItem("resq_phone") || "";
      const locationValue = (pending && pending.location) || localStorage.getItem("resq_location") || "";
      ...
      if (profileData && profileData.__raw) {
        console.error("Non-JSON response from /api/auth/profile:", profileData.__raw);
        throw new Error("Server returned unexpected response. Check backend logs (see console).");
      }
      ...
      if (!profileResponse.ok) {
        throw new Error(
          profileData && profileData.message ? profileData.message : "Unable to create your profile."
        );
      }
  ...
  const handleGoogleLogin =
    async () => {
      try {
        ...
        if (profileResponse.ok) {
        const data = await parseJsonSafe(profileResponse);

        if (data && data.__raw) {
          console.error("Non-JSON response from /api/auth/me:", data.__raw);
          throw new Error("Unable to verify profile: unexpected server response.");
        }
        ...
        if (
          ...
          if (createData && createData.__raw) {
            console.error("Non-JSON response from /api/auth/profile (create):", createData.__raw);
            throw new Error("Unable to create profile: unexpected server response.");
          }

          if (!createResponse.ok) {
            throw new Error(
              createData && createData.message ? createData.message : "Unable to create profile."
            );
          }
