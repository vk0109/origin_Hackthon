...
function Signup({ onLogin, onClose }) {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      if (data && data.__raw) {
        console.error("Non-JSON response from /api/auth/profile (google signup):", data.__raw);
        throw new Error("Profile creation failed: unexpected server response. Check backend logs.");
      }

      if (!response.ok) {
        throw new Error(
          data && data.message ? data.message : "Profile creation failed."
        );
      }
